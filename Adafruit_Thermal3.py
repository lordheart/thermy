...
class Adafruit_Thermal(Serial):
	...
	def __init__(self, port="/dev/ttyAMA0", baudrate=19200, timeout=5,
		...
		self.writeBytes(
		  27,       # Esc
		  55,       # 7 (print settings)
		  11,       # Heat dots (20 = balance darkness w/no jams)
	...
	def writeBytes(self, *args):
		...
		for arg in args:
			super(Adafruit_Thermal, self).write(struct.pack("B", arg))
	...
	def printBarcode(self, text, type):
		...
		super(Adafruit_Thermal, self).write(text.encode("utf-8"))
		self.prevByte = '\n'
		self.feed(2)
	...
	def print(self, *args, **kwargs):
		for arg in args:
			self.write(str(arg).encode("utf-8"))
