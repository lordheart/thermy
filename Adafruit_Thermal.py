...
def chunks(s, n): #yield 'n' -characters chunks from 's'.
...
class Adafruit_Thermal(Serial):
    ...
    barcodeHeight   = 50
    printMode       =  0
    defaultHeatTime = 255 #was 60 calibration tested up to 255

    def __init__(self, port="/dev/ttyAMA0", baudrate=19200, timeout=5,
        heattime=0, dtr=256):
        ...
        self.writeBytes(
          27,       # Esc
          55,       # 7 (print settings) 
          7,       # Heat dots (20 = balance darkness w/no jams) was 11
          heattime, # Lib default = 45
          40)      # Heat interval (500 uS = slower but darker)
    ...
    #prints string upside, in the correct order 
    def printUpsideDown(self, longString, charPerLine=32, linesToAchieve=6):
        split = []
